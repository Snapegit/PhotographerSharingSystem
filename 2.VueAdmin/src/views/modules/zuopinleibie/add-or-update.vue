<template>
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		>
			<template >
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 10px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="作品类别" prop="zuopinleibie">
					<el-input v-model="ruleForm.zuopinleibie" placeholder="作品类别" clearable  :readonly="ro.zuopinleibie"></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 10px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else class="input" label="作品类别" prop="zuopinleibie">
					<el-input v-model="ruleForm.zuopinleibie" placeholder="作品类别" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 10px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-if="type!='info' && !ro.image" label="image" prop="image">
					<file-upload
						tip="点击上传image"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 10px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-else-if="ruleForm.image" label="image" prop="image">
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `zuopinleibie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zuopinleibieCrossAddOrUpdateFlag = false;
											this.parent.search();
