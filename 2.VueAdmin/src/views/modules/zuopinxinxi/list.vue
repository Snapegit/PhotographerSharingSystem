<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"1px solid #ceddee","minHeight":"100px","padding":"20px 20px","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 20px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230810/3647a571588849539f384f974d3e6e28.jpg) no-repeat center top / cover","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"center","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">作品名称</label>
						<el-input v-model="searchForm.zuopinmingcheng" placeholder="作品名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="作品类别" prop="zuopinleibie">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">作品类别</label>
						<el-select clearable v-model="searchForm.zuopinleibie" placeholder="请选择作品类别" >
							<el-option v-for="(item,index) in zuopinleibieOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">摄影标签</label>
						<el-input v-model="searchForm.sheyingbiaoqian" placeholder="摄影标签" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					...
					<el-button class="btn18" v-if="isAuth('zuopinxinxi','作品类别统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						作品类别统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('zuopinxinxi','用户发布统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						用户发布统计
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"20px","boxShadow":"inset 0px 0px 32px 0px #E8EFF7,0 3px 6px 0px rgba(100,100,100,.1)","borderColor":"#ceddee","margin":"0 0 0px","borderRadius":"8px","background":"rgba(255,255,255,1)","borderWidth":"1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="作品类别">
						<template slot-scope="scope">
							{{scope.row.zuopinleibie}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="sheyingbiaoqian"
						label="摄影标签">
						<template slot-scope="scope">
							{{scope.row.sheyingbiaoqian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="zuopinfengmian" width="200" label="作品封面">
						<template slot-scope="scope">
							<div v-if="scope.row.zuopinfengmian">
								<img v-if="scope.row.zuopinfengmian.substring(0,4)=='http'" :src="scope.row.zuopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.zuopinfengmian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.zuopinfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.zuopinfengmian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="guangquandaxiao"
						label="光圈大小">
						<template slot-scope="scope">
							{{scope.row.guangquandaxiao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="kuaimensudu"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="view" v-if="isAuth('zuopinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan8" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								查看评论
							</el-button>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="zuopinleibieChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yonghumingChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var zuopinleibieChart1 = echarts.init(document.getElementById("zuopinleibieChart1"),'macarons');
        this.$http({
            url: "zuopinxinxi/group/zuopinleibie",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].zuopinleibie);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].zuopinleibie
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zuopinleibieChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zuopinleibieChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var yonghumingChart2 = echarts.init(document.getElementById("yonghumingChart2"),'macarons');
        this.$http({
            url: "zuopinxinxi/group/yonghuming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yonghumingChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yonghumingChart2.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.zuopinleibie!='' && this.searchForm.zuopinleibie!=undefined){
            params['zuopinleibie'] = this.searchForm.zuopinleibie
          }
           if(this.searchForm.sheyingbiaoqian!='' && this.searchForm.sheyingbiaoqian!=undefined){
            params['sheyingbiaoqian'] = '%' + this.searchForm.sheyingbiaoqian + '%'
          }
	...
	zuopinxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zuopinxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
